...
window.VIEW_BLOW = async function render(container) {
  ...
  try {
    const [trucksRes, historyRes] = await Promise.all([
      APP.getTrucks(),
      APP.getHistory('blow', today.substring(0, 7))
    ]);
    ...
    const blowContent = document.getElementById('blow-content');

    if (alreadySubmitted && alreadyDone.length > 0) {
      blowContent.innerHTML = `
          ...
          ${trucks.map(t => {
            ...
            return `<div class="truck-row ${done ? 'done' : ''}">
              ...
              <div class="truck-info">
                ...
                ${rec && rec.note ? `<div style="font-size:12px;color:#7f8c8d;margin-top:2px">${rec.note}</div>` : ''}
              </div>
            </div>`;
          }).join('')}
        </div>
        <button class="btn btn-outline btn-full" onclick="window.VIEW_BLOW_EDIT()">✏️ บันทึกเพิ่มเติม</button>
      `;
    } else {
      renderBlowForm(blowContent, trucks, today, alreadyDone);
    }

    window.VIEW_BLOW_EDIT = () => renderBlowForm(blowContent, trucks, today, alreadyDone);
  } catch (e) {
    APP.showError(document.getElementById('blow-content'), 'ไม่สามารถโหลดข้อมูลได้: ' + e.message);
  }
};

function renderBlowForm(container, trucks, today, alreadyDone) {
  ...
  container.innerHTML = `
    <div class="card">
      ...
      <div style="margin-bottom:10px;display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn btn-sm btn-outline" onclick="selectAllBlow(true)">เลือกทั้งหมด</button>
        <button class="btn btn-sm btn-outline" onclick="selectAllBlow(false)">ยกเลิกทั้งหมด</button>
      </div>
      <div class="truck-list" id="blow-list">
        ${activeTrucks.map((t, i) => {
          ...
          return `
            <div class="truck-row ${checked ? 'done' : ''}" id="blow-row-${i}">
              <input type="checkbox" class="truck-check blow-check" data-idx="${i}" ${checked ? 'checked' : ''}
                onchange="document.getElementById('blow-row-${i}').className='truck-row'+(this.checked?' done':'')">
              <div class="truck-info">
                <div class="truck-no">${t.truck_no}</div>
                <div class="truck-driver">${t.driver} ${APP.createStatusBadge(t.status)}</div>
              </div>
              <input type="text" class="truck-note" placeholder="หมายเหตุ" value="${note}" data-truck="${t.truck_no}">
            </div>`;
        }).join('')}
      </div>
    </div>
    <div id="blow-msg"></div>
    <button class="btn btn-primary btn-full" id="blow-submit-btn" onclick="submitBlow()">💾 บันทึกทั้งหมด</button>
  `;

  window._blowTrucks = activeTrucks;
  window._blowToday = today;

  window.selectAllBlow = (val) => {
    document.querySelectorAll('.blow-check').forEach((cb, i) => {
      cb.checked = val;
      const row = document.getElementById('blow-row-' + i);
      if (row) row.className = 'truck-row' + (val ? ' done' : '');
    });
  };

  window.submitBlow = async () => {
    const btn = document.getElementById('blow-submit-btn');
    const msg = document.getElementById('blow-msg');
    APP.setButtonLoading(btn, true);
    msg.innerHTML = '';

    const checkboxes = document.querySelectorAll('.blow-check');
    const notes = document.querySelectorAll('.truck-note');
    const records = window._blowTrucks.map((t, i) => ({
      truck_no: t.truck_no,
      driver: t.driver,
      status: t.status,
    ...
    try {
      await APP.saveBlow(records, window._blowToday);
